...
function HeadingLink(props: HeadingProps) {
  ...
  const sharedProps = {
    onClick: handleClick,
    get class() {
      return `${props.class ?? ""} group relative cursor-pointer hover:opacity-80`
    },
  ...
  const content = (
    <>
      <span
        class="pointer-events-none absolute top-1/2 -left-4 -translate-y-1/2 font-normal text-sm opacity-0 transition-opacity duration-150 group-hover:opacity-60"
        aria-hidden="true"
      >
        #
      </span>
  ...
  if ((props.level ?? 1) === 6)
    return (
      <h6 {...props} {...sharedProps}>
        {content}
      </h6>
    )
  if ((props.level ?? 1) === 5)
    return (
      <h5 {...props} {...sharedProps}>
        {content}
      </h5>
    )
  if ((props.level ?? 1) === 4)
    return (
      <h4 {...props} {...sharedProps}>
        {content}
      </h4>
    )
  if ((props.level ?? 1) === 3)
    return (
      <h3 {...props} {...sharedProps}>
        {content}
      </h3>
    )
  if ((props.level ?? 1) === 2)
    return (
      <h2 {...props} {...sharedProps}>
        {content}
      </h2>
    )
  return (
    <h1 {...props} {...sharedProps}>
      {content}
    </h1>
  )
}
